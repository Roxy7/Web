...
/*
 * This file is part of the SensioLabs Security Checker. (c) 2013 Fabien Potencier For the full copyright and license information, please view the LICENSE file that was distributed with this source code.
 */
...
class SecurityCheckerCommand extends Command {
	...
	protected function configure() {
		$this->setName ( 'security:check' )->setDefinition ( array (
		...
		) )->setDescription ( 'Checks security issues in your project dependencies' )->setHelp ( <<<EOF
...
EOF
	 ...
	 *        	The file path to the checked lock file
	...
	private function displayResults(OutputInterface $output, $lockFilePath, array $vulnerabilities) {
		...
		$output->writeln ( $this->getHelper ( 'formatter' )->formatBlock ( array (
				'[' . $status . ']',
				$count . ' packages have known vulnerabilities' 
		), $style, true ) );
		$output->write ( "\n" );
