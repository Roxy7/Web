...
class ScriptHandler {
	...
	public static function installRequirementsFile(CommandEvent $event) {
		...
		if (! $newDirectoryStructure) {
			...
			$fs->remove ( array (
					$appDir . '/check.php',
					$appDir . '/SymfonyRequirements.php',
					true 
			) );
	...
	public static function installAcmeDemoBundle(CommandEvent $event) {
		...
		$fs = new Filesystem ();
		$fs->mirror ( __DIR__ . '/../Resources/skeleton/acme-demo-bundle', $rootDir . '/src', null, array (
				'override' 
		) );
...
EOF;
	...
	public static function doBuildBootstrap($bootstrapDir, $autoloadDir = null, $useNewDirectoryStructure = false) {
		...
		if ($useNewDirectoryStructure) {
			...
			$bootstrapContent = str_replace ( array (
					"return \$this->rootDir.'/logs",
					"return \$this->rootDir.'/cache" 
			), array (
					"return \$this->rootDir.'/" . $cacheDir . "logs",
					"return \$this->rootDir.'/" . $cacheDir . "cache" 
			), $bootstrapContent );
		}
...
EOF;
		$phpunit = str_replace ( array (
				'<directory>../src',
				'"bootstrap.php.cache"',
				$phpunitKernelBefore 
		), array (
				'<directory>src',
				'"' . $varDir . '/bootstrap.php.cache"',
				$phpunitKernelAfter 
		), file_get_contents ( $rootDir . '/phpunit.xml.dist' ) );
		...
		$fs->dumpFile ( $binDir . '/console', str_replace ( array (
				".'/bootstrap.php.cache'",
				".'/AppKernel.php'" 
		), array (
				".'/" . $fs->makePathRelative ( $varDir, $binDir ) . "bootstrap.php.cache'",
				".'/" . $fs->makePathRelative ( $appDir, $binDir ) . "AppKernel.php'" 
		), file_get_contents ( $binDir . '/console' ) ) );
