...
class ConfiguratorController extends ContainerAware {
		...
		if ($request->isMethod ( 'POST' )) {
			...
			if ($form->isValid ()) {
				...
				if ($index < $configurator->getStepCount ()) {
					return new RedirectResponse ( $this->container->get ( 'router' )->generate ( '_configurator_step', array (
							'index' => $index 
					) ) );
				}
	...
	public function checkAction() {
		...
		$url = $this->container->get ( 'router' )->generate ( '_configurator_step', array (
				'index' => 0 
		) );
