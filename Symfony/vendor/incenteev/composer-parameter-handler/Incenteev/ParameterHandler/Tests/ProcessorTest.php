...
class ProcessorTest extends ProphecyTestCase {
	...
	public function testParameterHandling($testCaseName) {
		...
		$testCase = array_replace_recursive ( array (
				'title' => 'unknown test',
				'config' => array (
						'file' => 'parameters.yml' 
				),
	...
	private function setInteractionExpectations(array $testCase) {
		...
		foreach ( $testCase ['requested_params'] as $param => $settings ) {
			$this->io->ask ( sprintf ( '<question>%s</question> (<comment>%s</comment>): ', $param, $settings ['default'] ), $settings ['default'] )->willReturn ( $settings ['input'] )->shouldBeCalled ();
		}
	}
